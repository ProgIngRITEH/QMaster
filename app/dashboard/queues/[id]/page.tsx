...
export default function AdminQueueDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    const [{ data: q }, { data: e }, { count }] = await Promise.all([
      supabase.from("queues").select("*").eq("id", id).single(),
      supabase
        .from("queue_entries")
        .select("*")
        .eq("queue_id", id)
  ...
  async function toggleOpen() {
    ...
    setToggling(true);
    if (queue.is_open) {
      await supabase.rpc("close_queue", { p_queue_id: id, p_clear: false });
    } else {
      await supabase.rpc("open_queue", { p_queue_id: id });
    }
    setToggling(false);
  }
  ...
  async function markServed(entryId: string) {
    ...
    await supabase.rpc("recalculate_positions", { p_queue_id: id });
  }

  async function markNoShow(entryId: string) {
    if (!queue?.no_show_tracking) return;
    await supabase
      .from("queue_entries")
  ...
  async function clearQueue() {
    ...
    await supabase.rpc("close_queue", { p_queue_id: id, p_clear: true });
